...
--Q2--BEGIN What state in the US is buying more 'Samsung' cell phones? 
SELECT TOP 1
STATE
...
INNER JOIN DIM_MANUFACTURER AS W
ON Z.IDManufacturer = W.IDManufacturer
...
COUNT(IDCustomer) AS N0_OF_TRANS
FROM dim_location as X
INNER JOIN FACT_TRANSACTIONS AS Y
ON X.IDLocation = Y.IDLocation 
INNER JOIN DIM_MODEL AS Z
...
--Q4--BEGIN Show the cheapest cellphone
...
SELECT Model_Name, AVG(UNIT_PRICE) AS AVERAGE_PRICE FROM DIM_MODEL AS X
INNER JOIN DIM_MANUFACTURER AS Y
ON X.IDManufacturer = Y.IDManufacturer
WHERE Manufacturer_Name IN
( SELECT TOP 5 Manufacturer_Name FROM FACT_TRANSACTIONS AS X1
INNER JOIN DIM_MODEL AS Y1
ON X1.IDModel =Y1.IDModel
INNER JOIN DIM_MANUFACTURER AS Z1
ON Y1.IDManufacturer = Z1.IDManufacturer
...
SELECT x.IDModel,
Model_Name FROM DIM_MODEL AS X
INNER JOIN FACT_TRANSACTIONS AS Y
ON X.IDModel = Y.IDModel
group by Model_Name,x.IDModel
HAVING
...
SUM(Quantity) >= ALL (SELECT TOP 5  SUM(QUANTITY) FROM FACT_TRANSACTIONS WHERE YEAR(DATE) = 2009 group by IDModel ORDER BY SUM(QUANTITY) DESC)
AND
...
select * from(
	select top 1 *  From
	(
		select Top 2 Manufacturer_Name,YEAR(Date) as year, SUM(Quantity) AS Tot_sales  from DIM_MANUFACTURER as X
...
select * from(
	...
	order by Tot_sales desc) as t2
--Q8--END
--Q9--BEGIN Show the manufacturers that sold cellphone in 2010 but didn’t in 2009.
SELECT Manufacturer_Name FROM DIM_MANUFACTURER AS X
INNER JOIN DIM_MODEL AS Y
ON X.IDManufacturer = Y.IDManufacturer
INNER JOIN FACT_TRANSACTIONS AS Z
ON Y.IDModel = Z.IDModel
...
select top 100 X.customer_name,X.Year,X.Average_Price,X.Average_Quantity,
case when Y.year is not null
then  format(convert (decimal(8,2), (X.Average_Price-Y.Average_Price)) / convert(decimal (8,2), Y.Average_Price),'p') else null
end as 'Yearly % Change' from

(select Y.customer_name, year (X.date) as Year , avg(X.totalprice) as Average_Price ,avg(X.quantity) as Average_Quantity from FACT_TRANSACTIONS as X
left join  DIM_CUSTOMER as Y on  X.IDCustomer = Y.IDCustomer
where  X.IDCustomer in (select top 10 idcustomer from FACT_TRANSACTIONS group by IDCustomer order by sum (totalprice) desc)
group by Y.Customer_Name , year(X.date)
)
X left join
(select Y.customer_name , year(X.date) as Year , avg(X.totalprice)  as Average_Price ,avg(X.quantity) as Average_Quantity from FACT_TRANSACTIONS as X
left join  DIM_CUSTOMER as Y on Y.IDCustomer = Y.IDCustomer
where X.IDCustomer in(select top 10  idcustomer from  FACT_TRANSACTIONS group by IDCustomer order by sum(totalprice) desc)
group by  Y.Customer_Name, year(X.date))

Y on X.Customer_Name = Y.Customer_Name and Y.year = X.year-1
